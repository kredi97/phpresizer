...
abstract class ImageEngine {
	...
	protected function _checkParams (){
		...
		if (!is_string($this->params['path'])) {
			throw new ResizerException('path is not string');
		}
		
		if (!$this->params['cacheFile'] OR !is_string($this->params['cacheFile'])) {
			throw new ResizerException ('cacheFile is not string');
		}
		
		if (!$this->params['size'] OR !is_array($this->params['size'])) {
			throw new ResizerException ('size is not array');
		}
		
		if (!in_array(strtolower(pathinfo($this->params['path'],PATHINFO_EXTENSION)),$this->types)) {
			throw new ResizerException ('extensin  '.pathinfo($this->params['path'],PATHINFO_EXTENSION).' is not avalible');
		}
	}
